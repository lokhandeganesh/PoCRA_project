...
function shcrdocheck(rdovalue) {
	...
	if (rdovalue === "rdofarmer") {
		...
		if (villagecode === "-1") {
			...
			$.ajax({
				url: "http://gis.mahapocra.gov.in/weatherservices/meta/shcbasicinfo?vincode=" + villagecode + "&pinno=0", success: function (result) {
					console.log(result)
...
function loadCadastral() {
	...
	var layerName = text + "_" + document.getElementById("district").value;
	// alert(layerName)
	...
	if (paramValue === "-1") {
		...
		geojson1 = new ol.layer.Vector({
			...
			style: function (feature) {
				...
				if (checkSoilcard(paramValue, feature.get("pin")) === "true") {
...
function loadshcDetails() {
	...
	if (villagecode === "-1") {
		...
		document.getElementById("ptTable").innerHTML = '<table id="example" class="table table-striped table-bordered" style="width:100%;font-size: 10px;">\n\
            <thead>\n\
				<tr> <th style="width:20px;text-align: center;font-size: 12px">Sr. No.</th>\n\
				<th style="width:20px;text-align: center;font-size: 12px">District</th>\n\
				<th style="width:20px;text-align: center;font-size: 12px">Block</th>\n\
				<th style="width:20px;text-align: center;font-size: 12px">Village</th>\n\
				<th style="width:120px;text-align: center;font-size: 12px">Sample No.</th>\n\
				<th style="width:120px;text-align: center;font-size: 12px">SHC No.</th>\n\
				<th style="width: 300px;font-size: 12px;text-align: center;">Farmer Name</th>\n\
                <th style="width: 25px;text-align: center;font-size: 12px">Survey No.</th>\n\
			...
			url: "http://gis.mahapocra.gov.in/weatherservices/meta/shcbasicinfo?vincode=" + villagecode + "&pinno=0", success: function (result) {
				for (var i = 0; i < result.basicInfo.length; i++) {
					table.row.add([
						...
						'<button class="btn" id="btn" onclick=loadshcfarmerDetails("' + result.basicInfo[i].shc_no + '");>View Card </button>'
					]).draw(false);
				}
...
function checkSoilcard(vincode, pinno) {
	...
	request.open('get', 'http://gis.mahapocra.gov.in/dashboard_testing_api_2020_12_22/meta/shcavailable?vincode=' + vincode + '&pinno=' + pinno, /* async = */ false);
	request.send();
...
function loadshcfarmerDetails(shcno) {
	...
	$.ajax({
		url: "http://gis.mahapocra.gov.in/weatherservices/meta/farmershc?shcno=" + shcno, cache: false,
		beforeSend: function () {
		...
		complete: function () {
			...
			for (var i = 0; i < result.length; i++) {
				...
				var basicDetails = "";
				basicDetails = '<tr> <th colspan="5" style="width:100%;text-align: left;font-size: 12px;background-color:#c5d3d4">Soil Health Card Number - ' + result[i].shc_no + '</th></tr>' + '<tr style="font-size:13px"> <th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#ffdd00">Farmer’s Details</th>\n\
				</tr>\n\
				<tr style="font-size:12px"> <td colspan="1" style="text-align: left;font-size: 12px;background-color:#e6e5df">Farmer Name</td>\n\
				<td colspan="4" style="text-align: left;font-size: 12px;background-color:#e6e5df">'+ result[i].farmer_name + '</td>\n\
				</tr>\n\
				<tr style="font-size:12px"> <td colspan="1" style="text-align: left;font-size: 12px;background-color:#e6e5df">Farmer’s/Husband Name</td>\n\
				<td colspan="4" style="text-align: left;font-size: 12px;background-color:#e6e5df">'+ result[i].father_name + '</td>\n\
				</tr style="font-size:12px">'+ '<tr> <th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#ffdd00">Soil Sample Details</th>\n\
				</tr>\n\
				<tr style="font-size:12px"> <td colspan="2" style="text-align: left;font-size: 12px;background-color:#e6e5df">Date of Sample Collection </td>\n\
				<td colspan="3" style="text-align: left;font-size: 12px;background-color:#e6e5df">'+ result[i].sample_coll_date + '</td>\n\
				</tr>\n\
				<tr style="font-size:12px"> <td colspan="2" style="text-align: left;font-size: 12px;background-color:#e6e5df">Survey No., Khasra No./ Dag No.</td>\n\
				<td colspan="3" style="text-align: left;font-size: 12px;background-color:#e6e5df">'+ result[i].surveyno + ',' + result[i].khasrano + '</td>\n\
				<tr style="font-size:12px"> <td colspan="2" style="text-align: left;font-size: 12px;background-color:#e6e5df">Farm Size</td>\n\
				<td colspan="3" style="text-align: left;font-size: 12px;background-color:#e6e5df">'+ result[i].irrigated + '</td>\n\
				</tr>';
				var testResultTable = "";
				for (var j = 0; j < testResult.length; j++) {
					testResultTable = testResultTable + '<tr style="font-size:12px"><td>' + (j + 1) + '</td><td>' + testResult[j].testparamname + '</td><td>' + testResult[j].testvalue + '</td><td>' + testResult[j].unit + '</td><td>' + testResult[j].rating + '</td></tr>';
				}
				var fertrecommandationTable = "";
				var fertrecommandationTable1 = "";

				for (var k = 0; k < fertrecommandation.length; k++) {
					var combinationTable = ""; var combinationTable1 = "";
					for (var l = 0; l < combination.length; l++) {
						if (fertrecommandation[k].sr_no === combination[l].sr_no) {
							combinationTable = combinationTable + '<tr><td>' + combination[l].furt_comb_one_name + '</td><td>' + combination[l].furt_comb_one_quantity + '</td></tr>';
							combinationTable1 = combinationTable1 + '<tr><td>' + combination[l].furt_comb_two_name + '</td><td>' + combination[l].furt_comb_two_quantity + '</td></tr>';
						}
					}

					fertrecommandationTable = fertrecommandationTable + '<tr><td >' + (k + 1) + '</td><td >' + fertrecommandation[k].crop_variety + '</td><td><table>' + combinationTable + '</table></td><td >' + fertrecommandation[k].organic_fert + '<br>' + fertrecommandation[k].reference_yield + '</td><td >' + fertrecommandation[k].bio_fert + '<br>' + fertrecommandation[k].bio_fert_qty + '</td></tr>';

					fertrecommandationTable1 = fertrecommandationTable1 + '<tr><td >' + (k + 1) + '</td><td >' + fertrecommandation[k].crop_variety + '</td><td><table>' + combinationTable1 + '</table></td><td >' + fertrecommandation[k].organic_fert + '<br>' + fertrecommandation[k].reference_yield + '</td><td >' + fertrecommandation[k].bio_fert + '<br>' + fertrecommandation[k].bio_fert_qty + '</td></tr>';
				}
				...
				// }
				document.getElementById("shcfarmerdetails").innerHTML = '<table style="width:100%;font-size: 10px;border: 1px solid black">\n\
			<tr > <th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#eba834">Soil Health Card</th>'+ basicDetails + '</table><table style="width:100%;font-size: 10px;"><tr> <th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#ffdd00">Soil Test Results</th>\n\
			</tr>\n\
			<tr style="border: 1px solid black"> <th  style="text-align: left;font-size: 12px;background-color:#85c505;" width="10px">Sr.No</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Parameter</th>\n\
			 <th  style="text-align: left;font-size: 12px;background-color:#85c505">Test Value</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Unit</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Rating</th>\n\
			</tr>'+ testResultTable + ' </table><br>' +
					'<table style="width:100%;font-size: 10px;border: 1px solid black">\n\
			<tr > <th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#eba834">Please use option 1 or option 2, not both.</th><tr > <th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#ffdb4d">Option 1</th><tr style="border: 1px solid black"> <th  style="text-align: left;font-size: 12px;background-color:#85c505;" width="10px">Sr.No</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Crop Variety</th>\n\
			 <th  style="text-align: left;font-size: 12px;background-color:#85c505">Fertilizer Combination-1  (kg/ha)</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Organic Fertilizer & Quantity</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Bio Fertilizer & Quantity</th></tr>'+ fertrecommandationTable + '<tr ><th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#e6e6ff">OR</th></tr><tr><th colspan="5" style="width:100%;text-align: center;font-size: 12px;background-color:#ffdb4d">Option 2</th><tr style="border: 1px solid black"> <th  style="text-align: left;font-size: 12px;background-color:#85c505;" width="10px">Sr.No</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Crop Variety</th>\n\
			 <th  style="text-align: left;font-size: 12px;background-color:#85c505">Fertilizer Combination-2  (kg/ha)</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Organic Fertilizer & Quantity</th>\n\
			<th  style="text-align: left;font-size: 12px;background-color:#85c505">Bio Fertilizer & Quantity</th></tr>'+ fertrecommandationTable1 + '</table>';
			}
